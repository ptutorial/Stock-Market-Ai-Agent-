...
export class GeminiAdapter implements ProviderAdapter {
      ...
      const category = response.status === 429
        ...
        : response.status >= 500
          ...
          : response.status === 401 || response.status === 403
            ? 'AuthenticationError'
            : 'InvalidRequestError';
      throw new GatewayError(category, `Gemini request failed with HTTP ${response.status}`, response.status === 429 || response.status >= 500, retryAfterMs);
    }
    return response;
  }
  ...
  async discoverModels(account: AccountConfig, credential: string): Promise<ModelInfo[]> {
    ...
    return (data.models ?? []).filter((m: { name?: string }) => m.name?.startsWith('models/')).map((m: { name?: string; supportedGenerationMethods?: string[] }) => {
