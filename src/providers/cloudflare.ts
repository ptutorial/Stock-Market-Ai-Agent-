...
export class CloudflareWorkersAIAdapter implements ProviderAdapter {
  ...
  async generate(account: AccountConfig, request: GenerateRequest, model: string, credential: string, requestId: string): Promise<GenerateResult> {
    ...
    const response = await fetch(this.endpoint(account, model), { method: 'POST', headers: { ...this.headers(credential), 'x-request-id': requestId }, body: JSON.stringify({ messages: messages(request), max_tokens: request.options?.maxTokens, temperature: request.options?.temperature }), signal: request.options?.signal });
    const data = await readJson(response);
    const result = data.result ?? {};
    return { text: result.response ?? result.output_text ?? '', provider: this.name, accountId: account.id, model, usage: usageFromOpenAI(result), requestId, latencyMs: Date.now() - started };
  }
  async *stream(account: AccountConfig, request: GenerateRequest, model: string, credential: string, requestId: string): AsyncIterable<StreamChunk> {
    const response = await fetch(this.endpoint(account, model), { method: 'POST', headers: { ...this.headers(credential), 'x-request-id': requestId }, body: JSON.stringify({ messages: messages(request), stream: true }), signal: request.options?.signal });
  ...
  async discoverModels(): Promise<string[]> { return []; }
