...
export interface DataSourceMetadata { source: DataSourceName; fetchedAt: number; observedAt?: number; freshness: FreshnessStatus; fallback?: boolean; }
export interface DataSourceResult<T> { data: T | null; metadata: DataSourceMetadata; }
export interface RoutedData<T> { data: T; metadata: DataSourceMetadata; }
...
function result<T>(source: DataSourceName, data: T | null, observedAt: number | undefined, now: number, maxAge?: number): DataSourceResult<T> { const freshness: FreshnessStatus = data === null ? 'missing' : observedAt === undefined || maxAge === undefined ? 'unknown' : now - observedAt <= maxAge * 1000 ? 'fresh' : 'stale'; return { data, metadata: { source, fetchedAt: now, observedAt, freshness } }; }
...
export class LocalDBDataSource implements MarketDataSource {
  ...
  private readonly clock: () => number; private readonly freshnessSeconds: NonNullable<LocalDBDataSourceOptions['freshnessSeconds']>;
...
export class DataSourceRouter implements MarketDataProvider {
  ...
  private async routeDetailed<T>(method: string, symbol: string, call: (source: MarketDataSource) => Promise<DataSourceResult<T>>): Promise<RoutedData<T>> {
    ...
    for (const source of this.sources) {
      try {
        ...
        if (response.data !== null && !this.fallbackOn.has(response.metadata.freshness)) return { data: response.data, metadata: { ...response.metadata, fallback: previous !== undefined } };
  ...
  private route<T>(method: string, symbol: string, call: (source: MarketDataSource) => Promise<DataSourceResult<T>>): Promise<T> { return this.routeDetailed(method, symbol, call).then((result) => result.data); }
  ...
  quoteDetailed(symbol: string, exchange?: string) { return this.routeDetailed('quote', symbol, (s) => s.quote(symbol, exchange)); }
  historyDetailed(symbol: string, timeframe: string, limit?: number, exchange?: string) { return this.routeDetailed('history', symbol, (s) => s.history(symbol, timeframe, limit, exchange)); }
  technicalsDetailed(symbol: string, timeframe?: string, exchange?: string) { return this.routeDetailed('technicals', symbol, (s) => s.technicals(symbol, timeframe, exchange)); }
  fundamentalsDetailed(symbol: string, exchange?: string) { return this.routeDetailed('fundamentals', symbol, (s) => s.fundamentals(symbol, exchange)); }
  newsDetailed(symbol: string, limit?: number) { return this.routeDetailed('news', symbol, (s) => s.news(symbol, limit)); }
  sectorStrengthDetailed(symbol: string, exchange?: string) { return this.routeDetailed('sectorStrength', symbol, (s) => s.sectorStrength(symbol, exchange)); }
  riskDetailed(symbol: string, exchange?: string) { return this.routeDetailed('risk', symbol, (s) => s.risk(symbol, exchange)); }
}
