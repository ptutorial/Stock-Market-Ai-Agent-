...
export interface LocalDBDataSourceOptions {
  executor: LocalDBExecutor;
  freshnessSeconds?: Partial<Record<'quote' | 'history' | 'technicals' | 'fundamentals' | 'news' | 'sectorStrength' | 'risk', number>>;
  clock?: () => number;
}
...
function result<T>(source: DataSourceName, data: T | null, observedAt: number | undefined, now: number, maxAge?: number): DataSourceResult<T> {
  const freshness: FreshnessStatus = data === null
    ? 'missing'
    : observedAt === undefined || maxAge === undefined
      ? 'unknown'
      : now - observedAt <= maxAge * 1000
        ? 'fresh'
        : 'stale';
  return { data, metadata: { source, fetchedAt: now, observedAt, freshness } };
}

function observedAtFrom<T extends { timestamp?: number; asOf?: number }>(data: T | null): number | undefined {
  if (!data) return undefined;
  return data.timestamp ?? data.asOf;
}
...
export class LocalDBDataSource implements MarketDataSource {
  ...
  constructor(private readonly options: LocalDBDataSourceOptions) {
...
export interface DataSourceRouterOptions {
  ...
  priority?: DataSourceName[];
  fallbackOn?: FreshnessStatus[];
  onFallback?: (event: { method: string; symbol: string; from: DataSourceName; to: DataSourceName; reason: FreshnessStatus }) => void;
}
...
export class DataSourceRouter implements MarketDataProvider {
  ...
  private readonly fallbackOn: Set<FreshnessStatus>;
  private readonly onFallback?: DataSourceRouterOptions['onFallback'];
