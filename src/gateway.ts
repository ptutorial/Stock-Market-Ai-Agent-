...
export class LLMGateway {
  ...
  async generate(
    ...
    const request: GenerateRequest = {
      prompt,
      options: { ...options, task: options.task ?? task },
    };
    const candidates = await this.selectCandidates(request);
    if (!candidates.length) {
      throw new GatewayError(
        'ProviderUnavailableError',
        'No eligible provider/account/model is available',
      );
    }
      ...
      for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
        try {
          ...
          await this.usage.record({
            ...
            retry: attempt < this.config.maxRetries && normalized.retryable,
          });
  ...
  stream(
    ...
    return (async function* () {
      ...
      const requestId = randomUUID();
      try {
        for await (const chunk of candidate.adapter.stream(
          candidate.account,
          request,
          candidate.model,
          credential,
  ...
  private async selectCandidates(request: GenerateRequest): Promise<Candidate[]> {
    ...
    for (const provider of providerOrder) {
      ...
      for (const account of this.cfg.accounts.filter((item) => item.provider === provider)) {
        ...
        if (!state) continue;
        if (account.enabled === false || state.health === 'disabled') continue;
        if ((state.cooldownUntil ?? 0) > Date.now()) continue;
        if (![...required].every((capability) => account.capabilities.includes(capability))) continue;

        const models = request.options?.model
          ? account.models.filter((model) => model === request.options?.model)
          : account.models;
        if (!models.length) continue;

        const discovered = await adapter.discoverModels(account, await this.credentialStore.get(account.credentialRef));
  ...
  private score(account: AccountConfig, state: AccountState): number {
    switch (this.cfg.strategy ?? 'priority') {
      case 'lowest_utilization':
        return -(state.requests + state.tokens / 1000);
      case 'least_recently_used':
        return -(state.lastUsedAt ?? 0);
      case 'fastest':
        return account.priority ?? 0;
      case 'cheapest':
        return -((account.costPerMillionInput ?? 0) + (account.costPerMillionOutput ?? 0));
      case 'round_robin': {
