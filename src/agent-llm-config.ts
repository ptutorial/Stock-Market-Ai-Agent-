import type { AgentRole } from './modules/agents/registry.js';
import type { AgentModelPolicy, LLMProvider } from './modules/llm/agent/agent-llm.js';

const PROVIDERS: LLMProvider[] = ['gemini', 'groq', 'openrouter', 'cloudflare'];
function provider(value: string | undefined, fallback: LLMProvider): LLMProvider { return value && PROVIDERS.includes(value as LLMProvider) ? value as LLMProvider : fallback; }
function model(env: NodeJS.ProcessEnv, key: string, fallback: string): string { return env[key]?.trim() || fallback; }
function positiveInt(env: NodeJS.ProcessEnv, key: string, fallback: number): number { const value = Number(env[key]); return Number.isFinite(value) && value > 0 ? Math.floor(value) : fallback; }
function buildPolicy(env: NodeJS.ProcessEnv, role: AgentRole, name: string, primaryProvider: LLMProvider, primaryModelKey: string, primaryModelFallback: string, fallbackProvider: LLMProvider, fallbackModelKey: string, fallbackModelFallback: string, outputFallback: number): AgentModelPolicy {
  return { name, primary: { provider: provider(env[`${role.toUpperCase()}_LLM_PROVIDER`], provider(env.AGENT_LLM_PRIMARY_PROVIDER, primaryProvider)), model: model(env, `${role.toUpperCase()}_LLM_MODEL`, model(env, primaryModelKey, primaryModelFallback)) }, fallback: [{ provider: provider(env[`${role.toUpperCase()}_LLM_FALLBACK_PROVIDER`], provider(env.AGENT_LLM_FALLBACK_PROVIDER, fallbackProvider)), model: model(env, `${role.toUpperCase()}_LLM_FALLBACK_MODEL`, model(env, fallbackModelKey, fallbackModelFallback)) }], maxOutputTokens: positiveInt(env, `${role.toUpperCase()}_LLM_MAX_OUTPUT_TOKENS`, outputFallback) };
}
