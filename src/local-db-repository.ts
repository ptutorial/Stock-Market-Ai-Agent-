import type { FundamentalSnapshot, MarketDataProvider, MarketNewsItem, MarketQuote, OHLCVBar } from './market-data.js';
import type { LocalDBExecutor } from './data-sources.js';

export interface SqlRow { [key: string]: unknown; }
export interface SqlExecutor {
  query<T extends SqlRow>(sql: string, params?: readonly unknown[]): Promise<T[]>;
}
...
function requiredNumber(value: unknown, field: string): number {
  const parsed = number(value);
  if (parsed === undefined) throw new Error(`Invalid ${field} returned by local database`);
  return parsed;
}
function timestamp(value: unknown): number {
  ...
  if (!Number.isFinite(parsed)) throw new Error('Invalid timestamp returned by local database');
  return parsed;
}
...
export class SqlMarketDataRepository implements LocalDBExecutor {
  ...
  async quote(symbol: string, exchange = 'NSE'): Promise<MarketQuote | null> {
    const rows = await this.db.query('SELECT symbol, COALESCE(last, close) AS price, previous_close, total_traded_quantity, trade_date FROM stock_daily WHERE symbol = ? AND (? IS NULL OR series = ?) ORDER BY trade_date DESC LIMIT 1', [symbol, exchange, exchange]);
    const row = rows[0];
    if (!row) return null;
    const price = requiredNumber(row.price, 'price');
  ...
  async history(symbol: string, timeframe: string, limit = 100, _exchange?: string): Promise<OHLCVBar[] | null> {
    if (timeframe !== '1d' && timeframe !== 'day' && timeframe !== 'daily') throw new Error(`Unsupported local history timeframe: ${timeframe}`);
    const rows = await this.db.query('SELECT timestamp, open, high, low, close, volume FROM stock_histories WHERE stock_id = ? ORDER BY timestamp DESC LIMIT ?', [symbol, limit]);
    return rows.length ? rows.reverse().map((row) => ({ timestamp: timestamp(row.timestamp), open: requiredNumber(row.open, 'open'), high: requiredNumber(row.high, 'high'), low: requiredNumber(row.low, 'low'), close: requiredNumber(row.close, 'close'), volume: requiredNumber(row.volume, 'volume') })) : null;
  }

  async technicals(symbol: string, timeframe = '1d', exchange?: string) {
    const rows = await this.db.query('SELECT symbol, trade_date, close, rsi_14, sma_20, sma_50, sma_200, volume_spike_ratio FROM stock_daily WHERE symbol = ? ORDER BY trade_date DESC LIMIT 1', [symbol]);
    const row = rows[0];
    if (!row) return null;
  ...
  async sectorStrength(symbol: string, _exchange?: string) {
    const rows = await this.db.query('SELECT symbol, sector, relative_strength, sector_change_percent, market_change_percent, rank, updated_at FROM sector_strength WHERE symbol = ? ORDER BY updated_at DESC LIMIT 1', [symbol]);
    const row = rows[0];
    if (!row) return null;
    return { symbol: String(row.symbol ?? symbol), sector: String(row.sector), asOf: timestamp(row.updated_at), relativeStrength: number(row.relative_strength), sectorChangePercent: number(row.sector_change_percent), marketChangePercent: number(row.market_change_percent), rank: number(row.rank) };
  }
...
export function createLocalDBDataSource(db: SqlExecutor) {
  return new SqlMarketDataRepository(db);
}
