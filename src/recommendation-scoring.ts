...
function scoreNews(value: unknown): number { if (!Array.isArray(value) || value.length === 0) return 50; const scores: number[] = value.map((item) => { const sentiment = asRecord(item).sentiment; return sentiment === 'positive' ? 100 : sentiment === 'negative' ? 0 : 50; }); return scores.reduce((a, b) => a + b, 0) / scores.length; }
