...
export interface AgentLLMRequest { requestId: string; agentId: string; role: AgentRole; task: TaskType; systemPrompt: string; input: unknown; policy: AgentModelPolicy; provider?: LLMProvider; model?: string; capabilities?: Capability[]; tools?: ToolDefinition[]; maxInputTokens?: number; maxOutputTokens?: number; }
...
export class MultiProviderAgentLLM implements AgentLLMGateway {
  ...
  async generate(request: AgentLLMRequest): Promise<AgentLLMResponse> {
    ...
    for (let index = 0; index < candidates.length; index += 1) { const candidate = candidates[index]; try { const response = await this.gateway.generate({ ...request, provider: candidate.provider, model: candidate.model, policy: { ...request.policy, primary: candidate, fallback: [] } }); return { ...response, provider: response.provider ?? candidate.provider, model: response.model ?? candidate.model, fallback: index > 0 || response.fallback }; } catch (error) { lastError = error; } }
...
export class LLMGatewayAgentAdapter implements AgentLLMGateway {
  ...
  async generate(request: AgentLLMRequest): Promise<AgentLLMResponse> {
    ...
    const result = await this.gateway.generate(request.task, `SYSTEM: ${request.systemPrompt}\nUSER: ${input}`, { provider, model, task: request.task, capabilities: request.capabilities, tools: request.tools, maxTokens: request.maxOutputTokens ?? request.policy.maxOutputTokens, requestId: request.requestId });
