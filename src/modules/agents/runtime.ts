...
import { DEFAULT_AGENT_MODEL_POLICIES, LLMGatewayAgentAdapter, MultiProviderAgentLLM } from '../../agent-llm.js';
...
export class AgentRuntime {
  ...
  async run(agent: AgentDefinition, context: AgentContext): Promise<AgentResult> {
    ...
    const effectiveToolNames = context.evidenceOnly ? [] : agent.toolNames;
    const allowedTools = new Set(effectiveToolNames);
    const toolCalls: ToolCall[] = [];
    const toolResults: AgentResult['toolResults'] = [];
    let prompt = this.buildPrompt(agent, context, toolResults, effectiveToolNames);
    let output = '';
    ...
    for (let round = 0; round < Math.min(this.maxRounds, agent.maxToolRounds ?? this.maxRounds); round += 1) {
      ...
      const response = await this.llm.generate({
        ...
        capabilities: agent.task === 'structured_output'
          ? ['chat', 'structured_output', ...(effectiveToolNames.length ? ['tool_calling' as const] : [])]
          : effectiveToolNames.length ? ['chat', 'tool_calling'] : ['chat'],
        tools: this.options.tools.definitions(effectiveToolNames),
      });
      ...
      for (const call of calls) {
        if (!allowedTools.has(call.name)) throw new Error(`Agent ${agent.id} is not permitted to use tool ${call.name}`);
        const toolOutput = await this.options.tools.execute(call.name, call.arguments, { requestId, agentId: agent.id });
        toolResults.push({ tool: call.name, input: call.arguments, output: toolOutput });
      }
      prompt = this.buildPrompt(agent, context, toolResults, effectiveToolNames, output);
    }

    return { agentId: agent.id, role: agent.role, output, toolCalls, toolResults, structured };
  }

  private buildPrompt(agent: AgentDefinition, context: AgentContext, results: AgentResult['toolResults'], effectiveToolNames: string[], previous?: string): string {
    return [
      ...
      effectiveToolNames.length ? 'Use only the tools listed in the tool definitions. Do not fabricate tool results.' : 'Use only supplied evidence; no tool calls are available for this step.',
